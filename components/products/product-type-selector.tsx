...
export function ProductTypeSelector({ onSelect, selectedType, className }: ProductTypeSelectorProps) {
  const productTypesList = [
    { id: 'samplePack', mapTo: 'sample-pack', label: 'Sample Pack', description: 'Drums, loops, one-shots', gradient: 'from-purple-500 to-pink-500' },
    { id: 'presetPack', mapTo: 'preset-pack', label: 'Preset Pack', description: 'Synth/effect settings', gradient: 'from-blue-500 to-cyan-500' },
    { id: 'musicCourse', mapTo: 'music-course', label: 'Music Course', description: 'Structured lessons', gradient: 'from-green-500 to-emerald-500' },
    { id: 'coachingCall', mapTo: 'coaching-call', label: 'Coaching Call', description: '1-on-1 mentoring', gradient: 'from-orange-500 to-red-500' },
    { id: 'beatLease', mapTo: 'beat-lease', label: 'Beat Lease', description: 'License instrumentals', gradient: 'from-indigo-500 to-purple-500' },
    { id: 'workshop', mapTo: 'workshop', label: 'Workshop', description: 'Live group sessions', gradient: 'from-pink-500 to-rose-500' },
    { id: 'leadMagnet', mapTo: 'lead-magnet', label: 'Lead Magnet', description: 'Free content', gradient: 'from-cyan-500 to-blue-500' },
    ...
    <div className={cn("space-y-4", className)}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {productTypesList.map((type) => {
          ...
          return (
            <ProductTypeTooltip key={type.id} productTypeId={type.mapTo as keyof typeof productTypes}>
              <Card
                className={cn(
...
export function ProductTypeSelectorCompact({ onSelect, selectedType, className }: ProductTypeSelectorProps) {
  const productTypesList = [
    { id: 'samplePack', mapTo: 'sample-pack', label: 'Sample Pack', icon: Music },
    { id: 'presetPack', mapTo: 'preset-pack', label: 'Preset Pack', icon: Package },
    { id: 'musicCourse', mapTo: 'music-course', label: 'Music Course', icon: BookOpen },
    { id: 'coachingCall', mapTo: 'coaching-call', label: 'Coaching', icon: Headphones },
    { id: 'beatLease', mapTo: 'beat-lease', label: 'Beat Lease', icon: DollarSign },
