...
export function SocialScheduler({ storeId, userId }: SocialSchedulerProps) {
  ...
  const connectPlatform = (platform: string) => {
    ...
    const authUrls: Record<string, string> = {
      instagram: `https://www.facebook.com/v18.0/dialog/oauth?client_id=${process.env.NEXT_PUBLIC_FACEBOOK_APP_ID}&redirect_uri=${redirectUri}&state=${state}&scope=pages_manage_posts,pages_read_engagement,pages_show_list,business_management,instagram_basic,instagram_content_publish,instagram_manage_messages,pages_messaging&display=popup&response_type=code&auth_type=rerequest`,
