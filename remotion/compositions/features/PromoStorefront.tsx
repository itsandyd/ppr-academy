...
const Scene1_Hook: React.FC = () => {
  ...
  return (
    <CenterScene opacity={op} translateY={y} tint={C.red} seed={210}>
      <div style={{ transform: `scale(${scale})` }}>
        <div style={{ fontSize: 40, fontWeight: 900, fontFamily: F, color: C.white, textAlign: "center", marginBottom: 20 }}>
          Your store is
        </div>
        <div style={{ fontSize: 100, fontWeight: 900, fontFamily: F, color: C.red, lineHeight: 1, textAlign: "center", textDecoration: "line-through" }}>
          UGLY.
        </div>
      </div>
    </CenterScene>
...
// ─── SCENE 2: PROBLEM (4-10s) — Generic Templates ──────────────────────
const Scene2_Problem: React.FC = () => {
  const { op, y } = useExit(160, 180);
  const frame = useCurrentFrame();
  
  return (
    <CenterScene opacity={op} translateY={y} tint={C.darkGray} seed={211}>
      <FadeUp delay={5}>
      ...
      <FadeUp delay={20}>
        <div style={{ fontSize: 40, fontWeight: 700, color: C.white, fontFamily: F, textAlign: "center", marginBottom: 40 }}>
          Generic
          <br />
          <span style={{ color: C.red, fontWeight: 900 }}>Templates.</span>
        </div>
      </FadeUp>
...
// ─── SCENE 3: SOLUTION (10-20s) — Custom Storefronts ───────────────────
const Scene3_Solution: React.FC = () => {
  ...
  return (
    <CenterScene opacity={op} translateY={y} tint={C.primary} seed={212}>
      <FadeUp delay={10}>
        <div style={{ fontSize: 48, fontWeight: 900, fontFamily: F, color: C.white, marginBottom: 40 }}>
          Build <GradientText from={C.primary} to={C.cyan}>Custom.</GradientText>
        </div>
      </FadeUp>
...
// ─── SCENE 4: PROOF (20-28s) — Beautiful Design ────────────────────────
const Scene4_Proof: React.FC = () => {
  ...
  return (
    <CenterScene opacity={op} translateY={y} tint={C.cyan} seed={213}>
      <FadeUp delay={5}>
        <SectionLabel color={C.cyan}>THE RESULT</SectionLabel>
      </FadeUp>
...
// ─── SCENE 5: CTA (28-35s) — Build Yours ───────────────────────────────
const Scene5_CTA: React.FC = () => {
  return (
    <CenterScene tint={C.primary} seed={214}>
      ...
      <FadeUp delay={20}>
        <div style={{ fontSize: 40, fontWeight: 900, fontFamily: F, color: C.white, marginTop: 30, textAlign: "center" }}>
          Build
          <br />
          <GradientText from={C.primary} to={C.cyan}>Yours.</GradientText>
        </div>
      </FadeUp>
