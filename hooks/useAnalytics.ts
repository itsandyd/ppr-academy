...
interface AnalyticsEvent {
  eventType: 
    // Existing events
    | "page_view" | "product_view" | "course_view" | "purchase" | "download" 
    | "video_play" | "video_complete" | "lesson_complete" | "course_complete" 
    | "search" | "click" | "signup" | "login"
    // NEW: Creator funnel events
    | "creator_started" | "creator_profile_completed" | "creator_published" | "first_sale"
    // NEW: Learner activation events
    | "enrollment" | "return_week_2"
    ...
    | "dm_sent" | "cta_clicked" | "campaign_view"
    // NEW: System events
...
export function useAnalytics() {
  ...
  const trackCreatorStarted = useCallback(async (metadata?: { daw?: string; audience_size?: number }) => {
    if (!user?.id) return;
  ...
  const trackCreatorPublished = useCallback(async (resourceId: string, resourceType: "course" | "digitalProduct") => {
    if (!user?.id) return;
  ...
  const trackFirstSale = useCallback(async (resourceId: string, amountCents: number, currency: string = "USD") => {
    if (!user?.id) return;
  ...
  const trackCampaignView = useCallback(async (campaignId: string, variant?: string) => {
    if (!user?.id) return;
  ...
  const trackCTAClick = useCallback(async (campaignId: string, ctaUrl: string, variant?: string) => {
    if (!user?.id) return;
  ...
  const trackEmailEvent = useCallback(async (
    eventType: "email_sent" | "email_delivered" | "email_opened" | "email_clicked" | "email_bounced" | "email_complained",
    campaignId?: string
  ) => {
    if (!user?.id) return;
  ...
  const trackError = useCallback(async (errorCode: string, errorMessage: string) => {
    if (!user?.id) return;
