  ...
  const saveTemplate = useCallback(async () => {
    ...
    try {
      ...
      if (state.templateId) { await updateMutation({ id: state.templateId as Id<"digitalProducts">, ...params }); }
        ...
        const result = await createMutation({ ...params, title: state.data.title || "Untitled Template", storeId, userId: user.id, productType: "digital", productCategory: "mixing-template", price: state.data.pricingModel === "free_with_gate" ? 0 : (state.data.price ? parseFloat(state.data.price) : 0), followGateConfig });
