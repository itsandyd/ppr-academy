    ...
    try {
      ...
      if (state.templateId) {
        await updateMutation({
          id: state.templateId as Id<"digitalProducts">,
          ...baseParams,
        ...
        const result = await createMutation({ ...baseParams, title: state.data.title || "Untitled Template", storeId, userId: user.id, productType: "digital", productCategory: "mixing-template", price: state.data.pricingModel === "free_with_gate" ? 0 : (state.data.price ? parseFloat(state.data.price) : 0), followGateConfig });
