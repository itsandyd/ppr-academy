...
export function FollowGateForm() {
  ...
  const handleContinue = () => {
    ...
    router.push(`/dashboard/create/course?step=options${courseId ? `&courseId=${courseId}` : ""}`);
  };

  const handleBack = () => {
    const courseId = searchParams.get("courseId");
    router.push(`/dashboard/create/course?step=pricing${courseId ? `&courseId=${courseId}` : ""}`);
  };
