...
export function OptionsForm() {
  ...
  const [formData, setFormData] = useState({
    // Sharing & SEO
    enableSharing: true,
    seoTitle: state.data?.title || "",
    seoDescription: state.data?.description || "",
  ...
  const handleBack = () => {
    router.push(`/dashboard/create/course?step=course`);
  };
      ...
      <Card className="border-0 sm:border shadow-none sm:shadow-sm">
        ...
        <CardContent className="px-4 sm:px-6 space-y-6">
          ...
          <div>
            ...
            <Input
              id="seo-title"
              value={formData.seoTitle || state.data?.title || ""}
              onChange={(e) => handleInputChange("seoTitle", e.target.value)}
              placeholder={state.data?.title || "Course title for search engines"}
              className="mt-2 h-12 text-base"
            />
            <p className="text-xs text-muted-foreground mt-1">
              Auto-populated from course title. Customize for better SEO.
            </p>
          </div>
          ...
          <div>
            ...
            <Textarea
              id="seo-description"
              value={formData.seoDescription || state.data?.description || ""}
              onChange={(e) => handleInputChange("seoDescription", e.target.value)}
              placeholder={state.data?.description || "Course description for search engines"}
              rows={3}
              className="mt-2 text-base min-h-[80px] resize-none"
            />
            <p className="text-xs text-muted-foreground mt-1">
              Auto-populated from course description. Edit for better search visibility.
            </p>
          </div>
        </CardContent>
      </Card>
