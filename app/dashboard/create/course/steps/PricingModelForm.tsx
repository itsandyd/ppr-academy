...
import { PricingModelSelector } from "../../shared/PricingModelSelector";
...
export function PricingModelForm() {
  ...
  const handleContinue = () => {
    ...
    router.push(`/dashboard/create/course?step=${nextStep}${courseId ? `&courseId=${courseId}` : ""}`);
  };

  const handleBack = () => {
    const courseId = searchParams.get("courseId");
    router.push(`/dashboard/create/course?step=course${courseId ? `&courseId=${courseId}` : ""}`);
  };
