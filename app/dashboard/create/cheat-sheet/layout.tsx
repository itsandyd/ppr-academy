...
import { CheatSheetCreationProvider, useCheatSheetCreation } from "./context";
...
const steps: Step[] = [
  ...
  },
  {
    ...
    label: "Publish",
    icon: Sparkles,
    description: "Set pricing & go live",
    color: "from-green-500 to-emerald-500",
    estimatedTime: "1 min",
  },
];

function LayoutContentInner({ children }: CheatSheetLayoutProps) {
  const { user } = useUser();
  const searchParams = useSearchParams();
  const router = useRouter();
  const currentStep = searchParams.get("step") || "basics";

  const { state, canPublish, createCheatSheet, saveCheatSheet } = useCheatSheetCreation();
  ...
  const navigateToStep = (step: string) => {
    router.push(
      `/dashboard/create/cheat-sheet?step=${step}${state.productId ? `&productId=${state.productId}` : ""}`
    );
  };
  ...
  const handlePublish = async () => {
    const result = await createCheatSheet();
    if (result.success) {
  ...
  return (
    <div className="space-y-6">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-2xl font-bold">
                {state.productId ? "Edit" : "Create"} Cheat Sheet
              </h1>
              <Badge variant="secondary">Digital Content</Badge>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className="hidden lg:block">
          <div className="sticky top-24">
            <StorefrontPreview
              product={{
                title: state.data.title || "Untitled Cheat Sheet",
                description: state.data.description,
                price: state.data.pricingModel === "free_with_gate" ? 0 : (state.data.price || 5),
                imageUrl: state.data.thumbnail,
                productType: "cheat-sheet",
              }}
              store={
                store
                  ? {
      ...
      <ActionBar
        ...
        nextLabel="Continue"
        publishLabel="Publish Cheat Sheet"
        variant="compact"
        showProgress={true}
...
function LayoutContent({ children }: CheatSheetLayoutProps) {
