...
export function PackPricingForm() {
  ...
  const handleNext = async () => {
    ...
    if (state.data.pricingModel === "free_with_gate") {
      router.push(`/dashboard/create/pack?type=${packType}&step=followGate${state.packId ? `&packId=${state.packId}` : ''}`);
    } else {
  ...
  const handleBack = () => {
    const packType = state.data.packType || "sample-pack";
    router.push(`/dashboard/create/pack?type=${packType}&step=basics${state.packId ? `&packId=${state.packId}` : ''}`);
  };
