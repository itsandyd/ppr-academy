...
export default function CourseNotificationsPage() {
  ...
  const handleSendNotification = async (customCopy?: any) => {
    ...
    if (!confirm(`📢 Send notification to ${studentCount} enrolled student${studentCount !== 1 ? 's' : ''}?\n\nThey will receive:\n• In-app notification\n${sendEmail ? '• Email notification' : '• No email (disabled)'}`)) {
      return;
    }
  ...
  return (
    <div className="container mx-auto p-6 space-y-6 max-w-5xl">
      ...
      <Card className={changes.enrolledStudentCount > 0 ? "border-green-200 dark:border-green-800" : "border-amber-200 dark:border-amber-800"}>
        <CardContent className="pt-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-semibold text-lg">
                  {changes.enrolledStudentCount} Enrolled Student{changes.enrolledStudentCount !== 1 ? 's' : ''}
                </h3>
            ...
            <Badge variant={changes.enrolledStudentCount > 0 ? "default" : "secondary"} className="text-lg px-4 py-2">
        ...
        <CardContent className="space-y-4">
          {changes.hasChanges ? (
            <>
              ...
              <Alert className="bg-green-50 dark:bg-green-950 border-green-200 dark:border-green-800">
                ...
                <AlertDescription className="text-green-900 dark:text-green-100">
                  <span className="font-semibold">{changes.enrolledStudentCount} enrolled student{changes.enrolledStudentCount !== 1 ? 's' : ''}</span> will receive this notification.
                  {changes.enrolledStudentCount === 0 && (
            ...
            <Alert>
              <AlertCircle className="h-4 w-4" />
              <AlertDescription>
                No changes detected since your last notification. Add new content to notify students!
              </AlertDescription>
            </Alert>
          )}
        </CardContent>
          ...
          <CardContent className="space-y-4">
            {!generatedCopy ? (
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  {editMode ? (
                    ...
                    <div className="p-4 bg-muted rounded-lg">
                      <p className="text-foreground whitespace-pre-wrap">{currentCopy.message}</p>
                    </div>
                  )}
                </div>
                      ...
                      <div className="space-y-2">
                        ...
                        {editMode ? (
                          <Input
                            ...
                            onChange={(e) => setEditedCopy({ ...editedCopy, emailSubject: e.target.value })}
                          />
                        ) : (
                          <div className="p-3 bg-muted rounded-lg">
                      ...
                      <div className="space-y-2">
                        ...
                        {editMode ? (
                          <Input
                            ...
                            onChange={(e) => setEditedCopy({ ...editedCopy, emailPreview: e.target.value })}
                          />
                        ) : (
                          <div className="p-3 bg-muted rounded-lg">
                            <p className="text-muted-foreground text-sm">{currentCopy.emailPreview}</p>
                          </div>
                        )}
                      </div>
                    </>
                ...
                {/* Send Actions */}
                <div className="flex items-center justify-between gap-4 p-4 bg-blue-50 dark:bg-blue-950 rounded-lg">
                  <div className="flex items-center gap-2">
                    <Users className="h-5 w-5 text-blue-600" />
                    <div>
                      <p className="font-medium text-blue-900 dark:text-blue-100">
                        Ready to send to {changes?.enrolledStudentCount || 0} enrolled student{(changes?.enrolledStudentCount || 0) !== 1 ? 's' : ''}
                      </p>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Past notifications sent to your students
          </CardDescription>
        </CardHeader>
        <CardContent>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {notification.changes.newModules > 0 && (
                      <Badge variant="outline">
                        +{notification.changes.newModules} modules
                      </Badge>
                    )}
                    {notification.changes.newLessons > 0 && (
                      <Badge variant="outline">
                        +{notification.changes.newLessons} lessons
                      </Badge>
                    )}
                    {notification.changes.newChapters > 0 && (
                      <Badge variant="outline">
                        +{notification.changes.newChapters} chapters
                      </Badge>
                    )}
                  </div>
                </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="grid grid-cols-3 gap-4 text-center text-sm">
              <div>
                <div className="font-bold text-lg">{changes.currentState.totalModules}</div>
                <div className="text-muted-foreground">Modules</div>
              </div>
              <div>
                <div className="font-bold text-lg">{changes.currentState.totalLessons}</div>
                <div className="text-muted-foreground">Lessons</div>
              </div>
              <div>
                <div className="font-bold text-lg">{changes.currentState.totalChapters}</div>
