...
export function PackPricingForm() {
  ...
  const handleNext = async () => {
    ...
    if (state.data.pricingModel === "free_with_gate") {
      router.push(`/store/${storeId}/products/pack/create?step=followGate${state.packId ? `&packId=${state.packId}` : ''}`);
    } else {
      router.push(`/store/${storeId}/products/pack/create?step=files${state.packId ? `&packId=${state.packId}` : ''}`);
    }
  };

  const handleBack = () => {
    router.push(`/store/${storeId}/products/pack/create?step=basics${state.packId ? `&packId=${state.packId}` : ''}`);
  };
