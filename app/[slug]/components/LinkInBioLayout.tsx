...
function LeadMagnetCard({ leadMagnet, storeData }: { leadMagnet: Product; storeData?: StoreData }) {
    ...
    <div>
      ...
      {isOpen && (
        <div
          ...
          style={{ backgroundColor: 'hsl(var(--background))', opacity: 1, pointerEvents: 'auto' }}
        >
...
function ProductCard({ product, storeData }: { product: Product; storeData?: StoreData }) {
  ...
  const handleClick = () => {
    if (isLeadMagnet) {
      ...
      toast({
        title: "Coming Soon!",
        description: `Digital product checkout for "${product.title}" is currently in development.`,
    ...
    <div>
      <Card
        className={`p-4 border hover:shadow-md transition-all cursor-pointer touch-manipulation ${
          isCourse ? "border-emerald-200 bg-gradient-to-r from-emerald-50 to-teal-50" : "border-gray-200"
        }`}
        onClick={handleClick}
      >
        <div className="flex items-center gap-3">
          <div className={`w-12 h-12 rounded-lg flex items-center justify-center flex-shrink-0 overflow-hidden ${
            isCourse ? "bg-emerald-100" : "bg-blue-100"
          }`}>
            {product.imageUrl ? (
              <Image
          ...
          <div className="flex-1 min-w-0">
            <h3 className={`font-semibold text-sm truncate ${isCourse ? "text-emerald-800" : "text-foreground"}`}>
              {product.title}
            </h3>
            <p className={`text-xs truncate ${isCourse ? "text-emerald-600" : "text-muted-foreground"}`}>
              {isCourse ? `Course • ${product.description || "Learn essential skills"}` : (product.description || "Digital Product")}
            </p>
          </div>
          <div className="flex items-center gap-1 flex-shrink-0">
            <Badge
              variant={isCourse ? "default" : "secondary"}
              className={`text-xs ${isCourse ? "bg-emerald-100 text-emerald-800 hover:bg-emerald-100" : ""}`}
            >
              ${product.price}
            </Badge>
            {isLeadMagnet ? (
  ...
  return (
    <div className="w-full space-y-3">
      ...
      {leadMagnets.map((leadMagnet) => (
        <LeadMagnetCard key={leadMagnet._id} leadMagnet={leadMagnet} storeData={storeData as StoreData} />
      ))}

      {/* Other Products */}
      {otherProducts.map((product) => (
        <ProductCard key={product._id} product={product} storeData={storeData as StoreData} />
      ))}
    </div>
  );
}
